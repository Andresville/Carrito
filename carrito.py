#Importamos sqlite3 y flask
import sqlite3
from flask import Flask, jsonify, request
from flask_cors import CORS

#Configuramos la conexion a la base de datos SQLite
DATABASE = 'inventario1.db'
...
#Metodo Modificar
    def modificar (self, nuevadescripcion, nuevacantidad, nuevoprecio):
...
#Metodo Consultar Producto del Inventario
...
#Metodo Modificar Inventario
    def modificar_producto (self, codigo, nuevadescripcion, nuevacantidad, nuevoprecio):
        producto = self.consultar_producto(codigo)
        if producto:
            producto.modificar(nuevadescripcion, nuevacantidad, nuevoprecio)
            self.cursor.execute("UPDATE productos SET descripcion = ?, cantidad = ?, precio = ? WHERE codigo = ?", (nuevadescripcion, nuevacantidad, nuevoprecio, codigo))
            self.conexion.commit()
            return jsonify({'message': 'Producto modificado correctamente.'}), 200
        return jsonify({'message': 'Producto no encontrado.'}), 404

#Metodo Listar Productos en Inventario
...
#Metodo Eliminar Producto de Inventario
...
#Metodo para agregar productos del Inventario al Carrito
    def agregar(self, codigo, cantidad, Inventario):
        #Primero confirmamos que el producto existe
        producto = Inventario.consultar_producto(codigo)
        if producto is None:
            return jsonify({'message': 'El producto no existe.'}), 404
        
        #Segundo verificamos que haya la cantidad requerida
        if producto.cantidad < cantidad:
            return jsonify({'message': 'Cantidad en stock insuficiente.'}), 400
        
        #Tercero si el producto existe y hay cantidad verificamos si ya fue agregado al carrito
        for items in self.items:
            if items.codigo == codigo:
                items.cantidad += cantidad
                #Descontamos los elementos del inventario
                self.cursor.execute("UPDATE productos SET cantidad = cantidad - ? WHERE codigo = ?", (cantidad, codigo))
                self.conexion.commit()
                return jsonify({'message': 'Producto agregado al carrito correctamente.'}), 200
            
        #Cuarto si no esta en el carrito los agregamos
...
#Metodo para descontar productos o eliminar del carrito
    def quitar(self, codigo, cantidad, Inventario):
        for item in self.items:
            if item.codigo == codigo:
                ...
                #Agregamoas los elementos al inventario
                self.cursor.execute("UPDATE productos SET cantidad = cantidad + ? WHERE codigo = ?", (cantidad, codigo))
                self.conexion.commit()
                return jsonify({'message': 'Producto quitado del carrito correctamente.'}), 200
            
        #Si el bucle finaliza sin realizar cambios es que el producto no estaba en el carrito
        return jsonify({'message': 'El producto no se encuentra en el carrito.'}), 404

#Metodo para mostrar los productos del carrrito
    def mostrar(self):
        productos_carrito = []
        for item in self.items:
